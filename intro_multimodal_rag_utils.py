...
def display_images(
    ...
    Returns:
        None (displays images using PIL's show() method or IPython display if available).
    """
...
def create_get_metadata_df(path:str="data/",model="gemini-2.5-flash")->Tuple[pd.DataFrame, pd.DataFrame]:

    # Specify the PDF folder with multiple PDFs to process( <50 pages each)
    ...
    text_metadata_df, image_metadata_df = get_document_metadata(
        model,  # we are passing "gemini-2.5-flash" model
        pdf_folder_path,
